...
folder are mapped to the application's root namespace, 'EmergenceDbtool'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "EmergenceDbtool"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'EmergenceDbtool' in the same way
as `"EmergenceDbtool/sass/src"`.
